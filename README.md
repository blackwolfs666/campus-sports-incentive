...
本项目为校园运动激励与活动管理系统，涉及用户身份、步数、活动报名、打卡内容、图片上传、奖品领取记录等数据。若用于真实部署，请根据实际业务场景完善用户授权、隐私政策、数据最小化、数据保留周期和删除机制。

本项目不应提交真实 `.env` 文件、数据库密码、微信 AppSecret、JWT Secret、真实用户数据、运行日志和运行时上传文件。
...
如启用 CAS/IDS 绑定功能，用户账号密码仅应用于本次认证请求，系统不应保存用户密码，也不应在日志中输出账号、密码、ticket、bind token 等敏感认证信息。生产环境必须使用 HTTPS，并妥善配置微信小程序合法域名、后端 CORS、数据库账号权限和日志脱敏策略。

`database/seed.sql` 仅用于本地开发和演示，不代表真实用户、真实活动或真实获奖记录。
